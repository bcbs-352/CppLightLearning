...
Scene *GenerateScene()
{
    ...
    // Circle *c1 = new Circle({0.5f, 0.3f}, 0.3f);
    // Shape *sli = new ShapeIntersect(c1, l1);
    float refract[3] = {1.5f, 1.5f, 1.5f};
    // Entity *e4 = new Entity(sli, {0.f, 0.f, 0.f}, 0.2f, 1.f, reftact);
    Shape *triangle = GeneratePolygon({{0.15f, 0.4f}, {0.5f, 0.3f}, {0.2f, 0.15f}});
    Entity *e4 = new Entity(triangle, {0.01f, 0.12f, 0.2f}, 0.2f, 0.75f, refract);
    Circle *CL = new Circle({0.3f, -0.5f}, 0.05f);
    Entity *Light = new SpotLight(CL, {30.f, 30.f, 30.f});
    return new Scene({e1, e2, e3, e4, Light});
}
