...
protected:
    ...
    void CalcIntersect(Point p, Vector d, Point &inter)
    {
        ...
        if (d.x)
            inter.y = (inter.x - p.x) * d.y / d.x + p.y;
        else
            inter.y = (-m_c - m_a * inter.x) / m_b;
    }
...
public:
    ...
    Line(Point p1, Point p2, Point inter)
    {
        ...
        if (!IsInside(inter)) //若inter在半平面下方，则翻转半平面
        {
            m_a = -m_a;
            m_b = -m_b;
            m_c = -m_c;
...
public:
    ...
    {
        return (m_o.x - m_r > left) && (m_o.x + m_r < right) && (m_o.y - m_r > up) && (m_o.y + m_r < down);
    }
};
//形状的并集
